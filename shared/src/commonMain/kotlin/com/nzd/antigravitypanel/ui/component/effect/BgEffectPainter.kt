...
/**
 * 把 [OS3_BG_FRAG] 那支着色器包成一支 Compose `Brush`，并管着一整套 uniform 的更新。
 * 原样搬自 miuix 示例的 `BgEffectPainter.kt`。
 *
 * 每个 `update*` 都带一次"跟上次一样就跳过"的判断：这些东西每帧都会被调用一遍，
 * 而 `setFloatUniform` 是有开销的。
 */
internal class BgEffectPainter {
    ...
    private companion object {
        const val U_TRANSLATE_Y = 0f
        const val U_ALPHA_MULTI = 1f
        const val U_NOISE_SCALE = 1.5f
        const val U_POINT_RADIUS_MULTI = 1f
    }
