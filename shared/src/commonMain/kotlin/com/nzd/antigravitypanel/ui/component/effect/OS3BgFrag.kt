...
/**
 * OS3 那套动效背景的 AGSL 片元着色器。原样搬自 miuix 示例的 `OS3BgFrag.kt`。
 *
 * 它干的事：四个色斑（`uPoints` + 每帧算出来的 `uPointsAnim`）按到有色中心的
 * 距离 `smoothstep` 叠在一起，再叠一层 Perlin 噪声调饱和度/明度，最后抖一点噪点
 * 消 banding。**没有圆矩形 / 渐隐的裁剪**——那些是 Compose 侧用 `alpha` 和
 * `drawRect` 干的，着色器只负责颜色。
 */
