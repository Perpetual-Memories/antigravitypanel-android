...
/**
 * 背景 Several 套预设参数。原样搬自 miuix 示例的 `BgEffectConfig.kt`。
 *
 * 每个 `Config` 是一组：色斑位置 `points`（每个 3 个数：x, y, 半径系数）、
 * 三套配色 `colors1/2/3`（每套 4 个 RGBA，共 16 个 float）、跑色一轮的秒数
 * `colorInterpPeriod`，以及明度/饱和度/位移三个偏移。
 *
 * OS2 那几套的三份配色是**同一个对象**（`animatesColors` 判定为不变，就不跑颜色动画），
 * OS3 的三份不同，所以每 [Config.colorInterpPeriod] 秒换一次色。我们用 OS3。
 */
