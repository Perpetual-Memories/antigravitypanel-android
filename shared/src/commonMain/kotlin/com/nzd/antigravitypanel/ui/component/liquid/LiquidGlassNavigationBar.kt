...
package com.nzd.antigravitypanel.ui.component.liquid
...
import com.nzd.antigravitypanel.ui.component.animation.DampedDragAnimation
import com.nzd.antigravitypanel.ui.component.animation.InteractiveHighlight
...
fun IosLiquidGlassNavigationBar(
    ...
    // 回调只走"用户操作"这一条路（下面的 clickable / onDragStopped），
    // 绝不从 [selectedIndex] 那条同步路径发出去。
    //
    // 原因是个真 bug：外部把 Pager 的当前页灌进 selectedIndex，而 Pager 在跨页动画
    // **走到一半**时 currentPage 就已经翻到相邻那页了。于是会发生
    // "点地图分布 → Pager 开始滚 → 中途翻到历史战绩 → 回灌 → 又回调一次切到历史战绩"，
    // 目标页就永远够不到。HyperIsland 那边靠 `pagerState.currentPage != index` 挡一下，
    // 但那只是把竞态窗口缩小，没关掉；这里从源头把回灌路径的回调拿掉。
    ...
    // 外部（Pager）改了当前页：只把指示器挪过去，不回调。
    LaunchedEffect(selectedIndex) {
        if (currentIndex != selectedIndex) currentIndex = selectedIndex
    }

    // 这里只负责指示器动画。"切页"由点击 / 拖动时直接回调，不再绕 effect 一圈——
    // 绕一圈的话要等 ~300ms 指示器动画跑完 Pager 才开始动，手感发木。
